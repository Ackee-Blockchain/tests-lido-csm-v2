...
from pytypes.tests.csm.MockTriggerableWithdrawalsGateway import (
    MockTriggerableWithdrawalsGateway,
)
...
class CSMFuzzTest(FuzzTest):
    ...
    mock_triggerable_withdrawals_gateway: MockTriggerableWithdrawalsGateway
    mock_locator: MockLocator
    ...
    node_operators: dict[uint, NodeOperator]
    ...
    def _deploy_or_upgrade(self):
        # use forked proxies
        self.module = CSModule("0xdA7dE2ECdDfccC6c3AF10108Db212ACBBf9EA83F")
        ...
        head, tail = abi.decode(
            self.module.call(abi.encode_with_signature("depositQueue()")),
            [uint128, uint128],
        )
        ...
        self.strikes = CSStrikes(OssifiableProxy.deploy(self.module, self.admin, b""))
        ...
        OssifiableProxy(self.module).proxy__changeAdmin(
            self.admin, from_=OssifiableProxy(self.module).proxy__getAdmin()
        )
        ...
        self.module.grantRole(
            self.module.DEFAULT_ADMIN_ROLE(), self.admin, from_=ORIGINAL_ADMIN
        )
    ...
    def pre_sequence(self) -> None:
        ...
        self.mock_triggerable_withdrawals_gateway = (
            MockTriggerableWithdrawalsGateway.deploy()
        )
        self.mock_locator = MockLocator.deploy(
            LIDO_LOCATOR, self.mock_triggerable_withdrawals_gateway
        )
        ...
        self.mock_triggerable_withdrawals_gateway.initialize(self.module)

        self.permissionless_gate = PermissionlessGate.deploy(self.module, self.admin)

        self.vetted_gate_factory = VettedGateFactory.deploy(
            VettedGate.deploy(self.module),
        )
        ...
        self.ejector = CSEjector.deploy(
            self.module, self.strikes, STAKING_MODULE_ID, self.admin
        )
        ...
        self.module.grantRole(
            self.module.CREATE_NODE_OPERATOR_ROLE(),
            self.permissionless_gate,
            from_=self.admin,
        )
        self.module.grantRole(
            self.module.CREATE_NODE_OPERATOR_ROLE(), self.vetted_gate, from_=self.admin
        )
        self.module.grantRole(
            self.module.REPORT_EL_REWARDS_STEALING_PENALTY_ROLE(),
            self.admin,
            from_=self.admin,
        )
        self.module.grantRole(
            self.module.SETTLE_EL_REWARDS_STEALING_PENALTY_ROLE(),
            self.admin,
            from_=self.admin,
        )
        self.module.grantRole(
            self.module.VERIFIER_ROLE(),
            self.verifier,
            from_=self.admin,
        )
        # needed for direct call of CSModule.onValidatorExitTriggered from TW gateway
        self.module.grantRole(
            self.module.STAKING_ROUTER_ROLE(),
            self.mock_triggerable_withdrawals_gateway,
            from_=self.admin,
        )
        self.accounting.grantRole(
        ...
        self.no_id = self.module.getNodeOperatorsCount()
        self.nonce = self.module.getNonce()
        ...
        for acc in itertools.chain(
            ...
            [no.rewards_account for no in self.node_operators.values()],
            [no.manager for no in self.node_operators.values()],
            chain.accounts,
        ):
    ...
    def _migrate_forked_node_operators(self):
        total_node_operators = self.module.getNodeOperatorsCount()
        for no_id in range(total_node_operators):
            no = self.module.getNodeOperator(no_id)
            keys, signatures = self.module.getSigningKeysWithSignatures(
            ...
            for i in range(no.totalAddedKeys):
                withdrawn[i] = self.module.isValidatorWithdrawn(no_id, i)
    ...
    def _change_manager(self, no: NodeOperator, new_manager: Account):
        if no.extended_permissions:
            if no.manager != new_manager:
                self.module.proposeNodeOperatorManagerAddressChange(
                    no.id, new_manager, from_=no.manager
                )
                self.module.confirmNodeOperatorManagerAddressChange(
            ...
            if no.rewards_account != new_manager:
                self.module.proposeNodeOperatorRewardAddressChange(
                    no.id, new_manager, from_=no.rewards_account
                )
                self.module.confirmNodeOperatorRewardAddressChange(
                    no.id, from_=new_manager
                )
    ...
    def _get_frame_info(self, timestamp: uint) -> tuple[uint, uint]:
        ...
        ) // EPOCHS_PER_FRAME * EPOCHS_PER_FRAME + self.initial_epoch
        ...
        if depositable_before != depositable:
            ...
            if depositable > enqueued:
                ...
                if priority < QUEUE_LOWEST_PRIORITY:
                    if into_priority > 0:
                        self.node_operators[no_id].used_priority_queue = True
                        assert self.module.getNodeOperator(no_id).usedPriorityQueue
    ...
    def flow_add_no(self) -> str | None:
        ...
        self.node_operators[self.no_id] = NodeOperator(
            id=self.no_id,
            curve_id=curve_id,
            keys=public_keys,
        ...
        if curve_id == VETTED_CURVE_ID:
            ...
            if self.vetted_season_active and referrer != sender:
                ...
                assert not any(
                    e for e in tx.events if isinstance(e, VettedGate.ReferralRecorded)
                )
        else:
            assert not any(
                e
        ...
        if priority < QUEUE_LOWEST_PRIORITY - 1:
            self.node_operators[self.no_id].used_priority_queue = True
    ...
    def flow_no_add_keys(self) -> str | None:
        ...
        depositable_before = self.module.getNodeOperator(
            no.id
    ...
    def flow_settle_stealing_penalty(self):
        ...
        for no in self.node_operators.values():
            assert self._get_actual_locked_bond(
                no, chain.blocks["pending"].timestamp
            ) == self.accounting.getActualLockedBond(no.id)

        tx = self.module.settleELRewardsStealingPenalty(
            list(self.node_operators.keys()), from_=self.admin
        )
    ...
    def flow_compensate_stealing_penalty(self):
        ...
        tx = self.module.compensateELRewardsStealingPenalty(
    ...
    def flow_clean_deposit_queue(self):
        depositable_keys = {
            no_id: self.module.getNodeOperator(no_id).depositableValidatorsCount
            for no_id in range(self.module.getNodeOperatorsCount())
        }
    ...
    def flow_migrate_to_priority_queue(self):
        ...
        with may_revert() as ex:
            tx = self.module.migrateToPriorityQueue(no.id, from_=random_account())
    ...
    def flow_obtain_deposit_data(self):
        ...
        t = chain.blocks["pending"].timestamp
        deposits_count = random_int(
            0,
            sum(
                self._get_depositable_keys(no, t) for no in self.node_operators.values()
            ),
        )
    ...
    def flow_submit_oracle_data(self):
        ...
        for _ in range(3):
            ...
            if N == 0 or distributed < N:
                ...
                node_operators.append(
                    random.sample(list(self.node_operators.keys()), N)
                )
            ...
            strikes_tree = MerkleTree()
            deposited_nos = [
                no for no in self.node_operators.values() if no.deposited_keys > 0
            ]
        ...
        for no_id in strikes[index].keys():
            for key_index, s in strikes[index][no_id].items():
                self.node_operators[no_id].key_strikes[key_index].append(uint(s))

                assert (
                    keccak256(
                        abi.encode(
                            uint(no_id),
                            self.node_operators[no_id].keys[key_index],
                            self.node_operators[no_id].key_strikes[key_index],
                        )
                    )
    ...
    def flow_update_target_validators_limit(self):
        ...
        tx = self.module.updateTargetValidatorsLimits(
    ...
    def flow_process_historical_withdrawal_proof(self):
        ...
        witness = CSVerifier.WithdrawalWitness(
            ...
            withdrawal_tree.get_proof(withdrawal_tree.leaves.index(withdrawal_root))
            + old_state_tree.get_proof(
                old_state_tree.leaves.index(withdrawal_tree.root)
            ),
            validator_tree.get_proof(validator_tree.leaves.index(validator_root))
            + old_state_tree.get_proof(
                old_state_tree.leaves.index(validator_tree.root)
            ),
        )
        ...
        historical_block_roots_tree = MerkleTree(
    ...
    def flow_process_withdrawal_proof(self):
        ...
        state_tree.add_leaf(random_bytes(32))  # historical summaries tree root
        assert len(state_tree.leaves) == 3
        ...
        witness = CSVerifier.WithdrawalWitness(
            ...
            validator.withdrawableEpoch,
            withdrawal_tree.get_proof(withdrawal_tree.leaves.index(withdrawal_root))
            + state_tree.get_proof(state_tree.leaves.index(withdrawal_tree.root)),
            validator_tree.get_proof(validator_tree.leaves.index(validator_root))
            + state_tree.get_proof(state_tree.leaves.index(validator_tree.root)),
        )
    ...
    def flow_process_bad_performance_proof(self):
        ...
        key_strikes = [
            CSStrikes.KeyStrikes(
                no_id, key_index, self.node_operators[no_id].key_strikes[key_index]
            )
            for no_id, key_index in nos
        ]
        key_strikes.sort(
            key=lambda x: self.strikes_tree.leaves.index(
                keccak256(
                    abi.encode(
                        ...
                        self.node_operators[x.nodeOperatorId].keys[x.keyIndex],
                        self.node_operators[x.nodeOperatorId].key_strikes[x.keyIndex],
                    )
                )
            )
        )
        ...
        for no_id, key_index in nos:
            ...
            if no.strikes_penalties[key_index] is None:
                ...
                assert (
                    CSExitPenalties.StrikesPenaltyProcessed(
                        no_id, no.keys[key_index], penalty
                    )
                ) in tx.events
            else:
                assert not any(
                    ...
                    and e.nodeOperatorId == no_id
                    and e.pubkey == no.keys[key_index]
                )
            ...
            if no.withdrawal_request_fees[key_index] is None:
                ...
                assert (
                    CSExitPenalties.TriggeredExitFeeRecorded(
                        no_id, 1, no.keys[key_index], withdrawal_request_fee, actual_fee
                    )
                    in tx.events
                )
            else:
    ...
    @flow()
    def flow_report_validator_exit_delay(self):
        nos: dict[int, list[int]] = defaultdict(list)
        ...
        no_id = random.choice(list(nos.keys()))
        no = self.node_operators[no_id]
        key_index = random.choice(nos[no_id])
        delay = random_int(95, 100) * 60 * 60

        with may_revert() as ex:
            tx = self.module.reportValidatorExitDelay(
                no.id, 0, no.keys[key_index], delay, from_=STAKING_ROUTER
            )

        exit_delay_penalty = self.parameters[no.curve_id].exit_delay_penalty

        if delay <= self.parameters[no.curve_id].allowed_exit_delay:
            assert ex.value == CSExitPenalties.ValidatorExitDelayNotApplicable()
            return "Validator exit delay not applicable"
        elif no.exit_delay_penalties[key_index] is not None:
            assert len(tx.events) == 0
            return "Validator exit delay already reported"
        else:
            assert ex.value is None
        ...
        logger.info(
            f"Reported validator exit delay for NO {no.id} and key index {key_index}"
        )
    ...
    def invariant_node_operators(self):
        with chain.snapshot_and_revert():
            for no in self.node_operators.values():
                ...
                self.module.updateDepositableValidatorsCount(no.id)
            ...
            for no in self.node_operators.values():
                assert b"".join(no.keys) == self.module.getSigningKeys(
                    no.id, 0, no.total_keys
                )
                assert (
                    b"".join(no.keys),
                    b"".join(no.signatures),
                ) == self.module.getSigningKeysWithSignatures(no.id, 0, no.total_keys)
                info = self.module.getNodeOperator(no.id)
                ...
                assert (
                    self.module.getNodeOperatorNonWithdrawnKeys(no.id)
                ...
                for key in range(no.total_keys):
                    assert no.withdrawn[key] == self.module.isValidatorWithdrawn(
                        no.id, key
                    )
            ...
            assert self.module.getStakingModuleSummary() == (
                exited_sum,
                deposited_sum,
                depositable_sum,
            )
    ...
    def invariant_nonce(self):
        assert self.nonce == self.module.getNonce()

    @invariant(period=DEFAULT_INVARIANT_PERIOD)
    def invariant_total_bond_shares(self):
        assert self.accounting.totalBondShares() == sum(
            no.bond_shares for no in self.node_operators.values()
        )

    @invariant(period=DEFAULT_INVARIANT_PERIOD)
    def invariant_queue(self):
        for priority in range(QUEUE_LOWEST_PRIORITY + 1):
            head, tail = self.module.depositQueuePointers(priority)
            current = head
            i = 0

            while current != tail:
                item = self.module.depositQueueItem(priority, current)
                no_id = item >> (256 - 64)
            ...
            assert self.module.depositQueueItem(priority, tail) == 0
